...
func (e *Exporter) Collect(ch chan<- prometheus.Metric) {
	ch <- prometheus.MustNewConstMetric(e.up, prometheus.CounterValue, float64(stat.GetStat().Status))
	ch <- prometheus.MustNewConstMetric(e.uptime, prometheus.GaugeValue, time.Now().Sub(stat.GetStat().UpTime).Seconds())
	ch <- prometheus.MustNewConstMetric(e.version, prometheus.GaugeValue, e.ver, "version")

	ch <- prometheus.MustNewConstMetric(e.jobq, prometheus.CounterValue, float64(stat.GetStat().JobNum))
	ch <- prometheus.MustNewConstMetric(e.request, prometheus.CounterValue, float64(stat.GetStat().ReqNum))
	ch <- prometheus.MustNewConstMetric(e.success, prometheus.CounterValue, float64(stat.GetStat().SuccNum))
	ch <- prometheus.MustNewConstMetric(e.failed, prometheus.CounterValue, float64(stat.GetStat().ErrNum))
	ch <- prometheus.MustNewConstMetric(e.del_failed, prometheus.CounterValue, float64(stat.GetStat().DelErrNum))
	ch <- prometheus.MustNewConstMetric(e.recovery_init, prometheus.CounterValue, float64(stat.GetStat().RecovInitNum))
	ch <- prometheus.MustNewConstMetric(e.recovry_succ, prometheus.CounterValue, float64(stat.GetStat().RecovSuccNum))
	ch <- prometheus.MustNewConstMetric(e.recovery, prometheus.CounterValue, float64(stat.GetStat().RecovNum))
}

func WebMetricStart(address string, timeout time.Duration, ver float64) *http.Server {
	exporter := NewExporter(timeout, ver)
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(`<html>
				<head><title>logTransformer Exporter</title></head>
				<body>
				<h1>logTransformer Exporter</h1>
