...
func (p *WorkerPool) Run(cache *cache.CacheData, cp *kafka.ClientPool) {
	...
	stat.GetStat().SetStatus(stat.STATUS_RUNNING)
...
func (p *WorkerPool) Stop() {
	p.wkctx.shutdown()
	p.wg.Wait()
	stat.GetStat().SetStatus(stat.STATUS_STOP)
}

func (p *WorkerPool) worker(ctx *contexts, idx int, cache *cache.CacheData, cp *kafka.ClientPool) {
	defer p.wg.Done()
...
func (p *WorkerPool) recoveryWorker(ctx *contexts, idx int, cache *cache.CacheData, cp *kafka.ClientPool) {
	defer p.wg.Done()
	...
	for {
		isStop := func() bool {
			...
			var count int64
			for {
				if stat.GetStat().GetStatus() == stat.STATUS_RUNNING {
...
func mainHandle(w *JobWork, cache *cache.CacheData, cli *kafka.WrapClient) error {
...
func RecoveryHandle(w *JobWork, cache *cache.CacheData, cli *kafka.WrapClient) error {
	...
	stat.GetStat().Inc(stat.ST_RECOVERY_SUCC)
...
func (p *WorkerPool) InitRecoveryAll(cache *cache.CacheData) error {
	...
	for iter.Next() {
		if strings.HasPrefix(string(iter.Key()), PrefixQueue) {
			...
			stat.GetStat().Inc(stat.ST_RECOVERY_INIT)
